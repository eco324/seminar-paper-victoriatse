...
Modified_final_data <- Final_data %>%
  mutate(RIBUS_factor = as.factor(case_when(RIBUS == 1 ~ "Ride", RIBUS== 2 ~ "Doesn't_Ride")),
         HISP_factor = as.factor(case_when(HISP == 1 ~ "Hispanic", HISP== 2 ~ "Non_Hispanic")),
         OWN_factor = as.factor(case_when(OWN == 1 ~ "Homeowner", OWN== 2 ~ "Non_Homeowner")),
         LIC_factor = as.factor(case_when(LIC == 1 ~ "License", LIC== 2 ~ "No_License")),
         TRANS_factor = as.factor(case_when(TRANS== 1 ~ "Pass", TRANS== 2 ~ "No_Pass")))
```

```{r Adjust variables 6}
Modified_final_data<- Final_data %>%  
   mutate(TRANS_factor = as.factor(case_when(TRANS== 1 ~ "Pass", TRANS== 2 ~ "No_Pass"))) 
```

```{r Adjust variables 7}
Modified_final_data<- Final_data %>%  
  mutate(ENROL_factor = as.factor(case_when(ENROL == 1 ~ "Student", WORKS== 2 ~ "Non_Student"))) 
``` 

```{r Adjust variables 8}
Modified_final_data<- Final_data %>% 
  mutate(BIKFA_factor = as.factor(case_when(BIKFA == 1 ~ "Bike_Accessible", BIKFA == 2 ~ "No_Bike_Accessible")))  
``` 
...
Regression_1 <- lm(RIBUS ~ factor(OWN), data = Modified_final_data)  
...
Regression_2 <- lm(RIBUS ~ factor(OWN) + HHVEH + factor(LIC) + factor(TRANS) + factor(BIKFA), data = Modified_final_data) 
...
Regression_3 <- lm(RIBUS ~ factor(OWN) + factor(RACE) + factor(HISP), data = Modified_final_data) 
...
Regression_4 <- lm(RIBUS ~  factor(OWN) + factor(EDUCA), data = Modified_final_data) 
...
Regression_5 <- lm(RIBUS ~  factor(OWN) + factor(INCOME), data = Modified_final_data) 
...
Regression_6 <- lm(RIBUS ~  factor(OWN) + factor(EMPLY) + factor(ENROL), data = Modified_final_data) 
...
Regression_8 <- lm(RIBUS ~  factor(OWN) + AGE, data = Modified_final_data) 
