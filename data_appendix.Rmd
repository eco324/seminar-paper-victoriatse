...
Modified_final_data<- Final_data %>% 
   mutate(HISP_factor = as.factor(case_when(HISP == 1 ~ "Hispanic", HISP== 2 ~ "Non_Hispanic")))
```

```{r Adjust variables 3}
Modified_final_data<- Final_data %>% 
    mutate(OWN_factor = as.factor(case_when(OWN == 1 ~ "Homeowner", OWN== 2 ~ "Non_Homeowner"))) 
```

```{r Adjust variables 4}
Modified_final_data<- Final_data %>% 
  mutate(LIC_factor = as.factor(case_when(LIC == 1 ~ "License", LIC== 2 ~ "No_License")))  
```  
